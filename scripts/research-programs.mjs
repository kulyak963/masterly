...
Universities in many European countries show ONE tuition figure that is actually the EU/EEA-citizen rate, while a real non-EU student (our target audience) pays a HIGHER rate — sometimes 1.5-3x higher, sometimes the same, occasionally even lower. Actively search for "non-EU tuition"/"international (non-EU/EEA)"/"third-country nationals" tables. If only one figure is shown with no split, use it but set verified=false. verified=true when tuition+deadline+language are ALL confirmed for non-EU students on the university's OWN official site — different pages of that same site are fine (tuition rarely sits on the same page as the deadline), as long as all three clearly belong to the SAME admission cycle/intake year. Never mix this year's tuition with a stale deadline from a past cycle. Aggregators (Mastersportal, Studyportals, etc.) don't count as confirmation.
...
## Never fabricate
If a real figure can't be found, use your best-sourced estimate, set verified=false, explain what's missing in cons. If you found NOTHING usable at all, set tuition_eur to null rather than inventing a plausible-sounding number — a labeled "unknown" beats a confident-looking guess. verified=true when tuition+deadline+language are ALL confirmed for non-EU students on the university's OWN official site — different pages of that same site are fine, as long as all three clearly belong to the SAME admission cycle/intake year (never mix this year's tuition with a stale deadline from a past cycle). Aggregators don't count as confirmation. Same rule for gpa_min — most European programs (especially Dutch/Nordic) don't publish one at all; a plausible "3.0" when none was actually found is fabrication, not an estimate. Leave it null unless a page states an actual number.
