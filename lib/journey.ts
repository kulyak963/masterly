...
const PORTFOLIO_CLUSTERS = {
  tech: { task: 'Собрать GitHub с 2-3 читаемыми проектами', detail: 'у каждого — README на английском: что делает, как запустить.', noWorkAdvice: 'без опыта работы GitHub — твоё главное доказательство навыков.' },
  creative: { task: 'Собрать портфолио на Behance, 10-15 работ', detail: 'у каждой работы — 2-3 предложения о замысле, не просто картинка.', noWorkAdvice: 'для творческих направлений портфолио смотрят раньше диплома.' },
  business: { task: 'Заполнить LinkedIn на английском', detail: 'с описанием реальных задач и результатов, не просто копия CV.', noWorkAdvice: 'без опыта работы многие Business-программы почти наверняка потребуют GMAT.' },
  research: { task: 'Описать исследовательский опыт с методиками', detail: 'конкретные методики и результат, не «работал в лаборатории».', noWorkAdvice: 'даже курсовой проект с реальной методикой — твой главный аргумент.' },
  writing: { task: 'Собрать портфолио текстов, 3-5 лучших', detail: 'в один PDF или Google Doc, со ссылками, если публиковались онлайн.', noWorkAdvice: 'портфолио текстов важнее самого CV для этого направления.' },
  academic: { task: 'Переписать практику академическим языком', detail: 'что конкретно делал(а) и какой был результат, не должностная инструкция.', noWorkAdvice: 'стажировка/практика — единственный способ связать теорию с реальными задачами.' },
  generic: { task: 'Собрать 2-3 примера опыта по направлению', detail: 'то, что реально относится к программе, а не всё резюме подряд.', noWorkAdvice: 'собери максимум конкретики из учёбы — курсовые, проекты, олимпиады.' },
} as const
  ...
  return [
    {
      ...
      tasks: [
        ...
        { key: 'research.cold-email', title: 'Написать научным руководителям', meta: 'за 3-4 мес. до дедлайна',
          detail: '5-7 предложений, почему интересна именно его тема — заметно повышает шанс на research-программах.' },
    ...
    },
    {
      ...
      why: `GPA ${p.gpa} — ${p.gpa >= 4.0 ? 'выше среднего для Европы' : 'достаточно для большинства программ'}.`,
      tasks: [
        ...
        p.work === 'no'
          ? { key: 'profile.internship', title: 'Найти стажировку или research project', detail: pf.noWorkAdvice }
    ...
    },
    {
      ...
      tasks: [
        ...
        { key: 'docs.sop', title: 'Написать Statement of Purpose под каждый вуз', meta: 'не копия между вузами',
          detail: `почему именно эта программа — назови курс/лабораторию/профессора; ${pf.detail}` },
        ...
        { key: 'docs.order-translation', title: 'Заказать нотариальный перевод диплома и апостиля', meta: '1500-3000₽/документ, 2-3 недели',
    ...
    },
    {
      ...
      why: wantsHu
        ? (isPro ? 'Stipendium Hungaricum закрывается 15 января — и это ДВЕ отдельные подачи, не одна.'
          : 'У выбранной страны есть важный дедлайн по стипендии — общая часть бесплатна, детали в Реальность · PRO.')
    ...
    },
    {
      ...
      tasks: [
        ...
        { key: 'final.visa-apply', title: 'Подать на студенческую визу', meta: 'за 2-3 месяца до семестра',
          detail: 'национальная виза D — общий процесс похож для Шенгена, детали разные.' },
